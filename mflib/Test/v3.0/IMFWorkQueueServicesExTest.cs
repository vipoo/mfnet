...
namespace Testv30
{
    class IMFWorkQueueServicesExTest : COMBase, IMFSampleGrabberSinkCallback, IMFAsyncCallback
    {
        int WORKQUEUE_ID;
        const string WORKQUEUE_MMCSS_CLASS = "audio";
        IMFWorkQueueServicesEx m_workQueueServices;
        TestThing2 pTransformUnk = null;
        long m_topologyid;

        public void DoTests()
        {
            GetInterface(Program.File1);
            int hr;

            hr = m_workQueueServices.BeginRegisterPlatformWorkQueueWithMMCSSEx(0, "games", 0, 2, this, null);
            MFError.ThrowExceptionForHR(hr);

            int p;
            hr = m_workQueueServices.GetPlatformWorkQueueMMCSSPriority(0, out p);
            MFError.ThrowExceptionForHR(hr);
            ...
            hr = m_workQueueServices.GetTopologyWorkQueueMMCSSPriority(j, out p2);
            MFError.ThrowExceptionForHR(hr);
        }
        ...
        private void GetInterface(string pszFileName)
        {
            ...
            hr = pTopology.GetTopologyID(out m_topologyid);
            MFError.ThrowExceptionForHR(hr);

            object service;

            hr = (pSession as IMFGetService).GetService(MFServices.MF_WORKQUEUE_SERVICES, typeof(IMFWorkQueueServicesEx).GUID, out service);
            MFError.ThrowExceptionForHR(hr);

            m_workQueueServices = (IMFWorkQueueServicesEx)service;
        }
        ...
        private void CreateTopology(IMFMediaSource pSource, IMFActivate pSinkActivate, out IMFTopology ppTopo)
        {
            ...
            for (int i = 0; i < cStreams; i++)
            {
                ...
                if (majorType == MFMediaType.Audio && fSelected)
                {
                    ...
                    pTransformUnk = new TestThing2(WORKQUEUE_MMCSS_CLASS);
    ...
    public class TestThing2 : COMBase, IMFTransform, IDisposable, IMFRealTimeClientEx
    {
        ...
        {
            int hr = MFExtern.MFTUnregister(typeof(TestThing2).GUID);
            MFError.ThrowExceptionForHR(hr);
        }

        #endregion

        public TestThing2(string sClass)
        {
            TRACE("Constructor");
        ...
        #region IMFRealTimeClient methods
        ...
        public int RegisterThreadsEx(ref int pdwTaskIndex, string wszClassName, int lBasePriority)
        {
            m_didit |= 1;
            return S_Ok;
        }

        public int SetWorkQueueEx(int dwMultithreadedWorkQueueId, int lWorkItemBasePriority)
        {
            if (dwMultithreadedWorkQueueId != 0)
            {
                m_didit |= 4;
            }
            else
