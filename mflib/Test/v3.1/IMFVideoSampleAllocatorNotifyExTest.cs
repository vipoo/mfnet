...
namespace Testv31
{
    // Does not QI for IMFVideoSampleAllocatorNotifyEx, only IMFVideoSampleAllocatorNotify
    class IMFVideoSampleAllocatorNotifyExTest : COMBase, IMFAsyncCallback, IMFVideoSampleAllocatorNotifyEx, ICustomQueryInterface
    {
        public void DoTests()
        {
            ...
            CreateMediaSource(Program.File1);
        ...
        public HResult NotifyPrune(IMFSample pSample)
        {
...
#if DEBUG
            if (iid != typeof(IMFAsyncCallback).GUID)
            {
