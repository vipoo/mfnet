...
namespace Testv21
{
    class IMFVideoSampleAllocatorTest : COMBase, IMFAsyncCallback
    {
        public void DoTests()
        {
            ...
            IMFTopologyNode p;
            IMFVideoSampleAllocator vsa = null;
            object o = null;
            short s;
            ...
            for (short x = 0; x < s; x++)
            {
                ...
                if (hr >= 0)
                {
                    ...
                    if (ss != null)
                    {
                        ...
                        if (hr >= 0)
                        {
                            vsa = (IMFVideoSampleAllocator)o;

                            // IDirect3DDeviceManager9
                            Guid g = new Guid("a0cade0f-06d5-4cf4-a1c7-f3cdd725aa75");

                            IMFMediaTypeHandler ph;
                            hr = ss.GetMediaTypeHandler(out ph);
                            MFError.ThrowExceptionForHR(hr);

                            hr = ph.GetCurrentMediaType(out mt);
                            MFError.ThrowExceptionForHR(hr);

                            hr = gs.GetService(MFServices.MR_VIDEO_ACCELERATION_SERVICE, g, out o);
                            MFError.ThrowExceptionForHR(hr);
                            break;
                        }
                    }
            ...
            hr = vsa.InitializeSampleAllocator(3, mt);
            MFError.ThrowExceptionForHR(hr);

            IMFSample ps;

            // This will fail if InitializeSampleAllocator didn't succeed
            hr = vsa.AllocateSample(out ps);  
            MFError.ThrowExceptionForHR(hr);

            // See if we got a sample
            int i;
            hr = ps.GetSampleFlags(out i);
            MFError.ThrowExceptionForHR(hr);

            // Test next method
            hr = vsa.UninitializeSampleAllocator();
            MFError.ThrowExceptionForHR(hr);

            // Make sure the Uninitialize succeeded
            hr = vsa.AllocateSample(out ps);
            Debug.Assert(hr < 0);

            // This method does an immediate QI for IDirect3DDeviceManager9, so
            // we know immediately if it works.
            hr = vsa.SetDirectXManager(o);
            MFError.ThrowExceptionForHR(hr);
        }
